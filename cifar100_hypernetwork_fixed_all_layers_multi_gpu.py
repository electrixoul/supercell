'''
CIFAR-100 HyperNetwork ALL LAYERS VERSION - Multi-GPU Training
==============================================================
基于cifar100_hypernetwork_fixed_all_layers.py，针对双H100的多GPU并行训练版本
...
def train_epoch(model, device, train_loader, optimizer, epoch, rank, world_size, log_interval=50):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if rank == 0 and batch_idx % log_interval == 0:
            ...
            # Z信号监控（只在主进程）
            conv_norms = [
                model.module.z_signal_conv1.norm().item(),  # 注意：DDP包装后需要.module
...
def train_worker(rank, world_size, args=None):
    ...
    # 参数统计（只在主进程）
    if rank == 0:
        print("\nMulti-GPU ALL LAYERS HyperNetwork CNN parameters:")
    ...
    for epoch in range(epochs):
        ...
        if rank == 0:
            print(f"\nEpoch {epoch+1}/{epochs} - Multi-GPU HyperNetwork CNN")
            print("-" * 50)
        ...
        if rank == 0:
            if val_err < best_val_err:
                ...
                torch.save(model.module.state_dict(), 'cifar100_all_layers_hyper_multi_gpu_best.pt')
    ...
    if rank == 0:
        print("\n" + "="*70)
        print("FINAL RESULTS - Multi-GPU Version")
        print("="*70)
        
        # 加载最佳模型
        model.module.load_state_dict(torch.load('cifar100_all_layers_hyper_multi_gpu_best.pt'))
    ...
    if rank == 0:
        ...
        print(f"ALL LAYERS HyperNetwork (Multi-GPU) - Test Accuracy: {test_acc:.2f}%")
        ...
        print(f"\nUsed {world_size} GPUs for training")
...
def main():
    ...
    print("Starting CIFAR-100 ALL LAYERS HyperNetwork - Multi-GPU Version")
