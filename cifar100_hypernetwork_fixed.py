'''
CIFAR-100 HyperNetwork FIXED VERSION
Fixed the critical bug in weight generation - now generates correct number of parameters
'''
...
class HyperCNNCIFAR_Fixed(nn.Module):
    def __init__(self, f_size=5, in_size=32, out_size=64, z_dim=8):
        super(HyperCNNCIFAR_Fixed, self).__init__()
        self.f_size = f_size
        ...
        # Hypernetwork for generating conv2 weights - SIMPLIFIED VERSION
        # Z signal - small initialization
        self.z_signal = nn.Parameter(torch.randn(1, z_dim) * 0.01)
        
        # Direct mapping: z_signal -> conv2 weights (no intermediate steps)
        conv2_weight_size = out_size * in_size * f_size * f_size
        self.hyper_linear = nn.Linear(z_dim, conv2_weight_size)
        
        # Initialize hypernetwork with small weights
        nn.init.normal_(self.hyper_linear.weight, std=0.01)
        nn.init.constant_(self.hyper_linear.bias, 0.0)
        
        print(f"FIXED: Generating {conv2_weight_size} parameters for conv2 ({out_size}x{in_size}x{f_size}x{f_size})")
    ...
    def generate_conv2_weights(self):
        """Generate conv2 weights using hypernetwork - SIMPLIFIED VERSION"""
        # Direct generation: z_signal -> conv2 weights
        weights_flat = self.hyper_linear(self.z_signal)  # [1, out_size * in_size * f_size * f_size]
        
        # Reshape to conv filter format
        kernel = weights_flat.reshape(self.out_size, self.in_size, self.f_size, self.f_size)
        return kernel
...
def main():
    print("Starting CIFAR-100 HyperNetwork (FIXED VERSION)...")
    print("="*60)
    ...
    in_size = 32    
    out_size = 64   
    z_dim = 8       # Smaller z_dim initially
    ...
    # Train FIXED HyperNetwork CNN
    print("Training FIXED HyperNetwork CNN...")
    model_hyper = HyperCNNCIFAR_Fixed(f_size=f_size, in_size=in_size, out_size=out_size, z_dim=z_dim).to(device)
    optimizer_hyper = optim.Adam(model_hyper.parameters(), lr=lr)
    scheduler_hyper = optim.lr_scheduler.ExponentialLR(optimizer_hyper, gamma=0.995)
    
    print("\nFIXED HyperNetwork CNN parameters:")
    ...
    for epoch in range(epochs):
        ...
        print(f"\nEpoch {epoch+1}/{epochs} - FIXED HyperNetwork CNN")
        print("-" * 40)
        ...
        if val_err < best_val_err_hyper:
            ...
            torch.save(model_hyper.state_dict(), 'cifar100_fixed_hyper_best.pt')
    ...
    model_hyper.load_state_dict(torch.load('cifar100_fixed_hyper_best.pt'))
    ...
    broken_hyper_acc = 1.00  # Previous broken hypernetwork result
    
    print(f"Standard CNN - Test Accuracy: {std_test_acc:.2f}% (previous result)")
    print(f"Broken HyperNetwork CNN - Test Accuracy: {broken_hyper_acc:.2f}% (previous result)")
    print(f"FIXED HyperNetwork CNN - Test Accuracy: {hyper_test_acc:.2f}%")
    
    improvement_vs_std = hyper_test_acc - std_test_acc
    improvement_vs_broken = hyper_test_acc - broken_hyper_acc
    
    print(f"Improvement vs Standard CNN: {improvement_vs_std:+.2f}%")
    print(f"Improvement vs Broken HyperNet: {improvement_vs_broken:+.2f}%")
